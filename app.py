...
from flask import Flask, send_from_directory, render_template, request, redirect, send_from_directory, url_for, g, flash
...
class ItemForm(FlaskForm):
    ...
    image = FileField("Image", validators=[FileRequired(), FileAllowed(app.config["ALLOWED_IMAGE_EXTENSIONS"], "Image Only!")])
...
def edit_item(item_id):
    ...
    if item:
        ...
        if form.validate_on_submit():
            ...
            conn.commit()
            flash("Item {} has been successfully modified.".format(item["title"]), "success")
            print(item["id"])
        ...
        return render_template("edit.html", editItemForm=form, item=item)
    else:
...
def home():
    ...
    # validate method for GET request
    if form.validate():
...
def new_item():
    ...
    subcategories = c.fetchall()
    # [(1, 'Fruit'), (2, 'Daily product'), (3, 'Cassette'), (4, 'Phone'), (5, 'TV'), (6, 'Historical fiction'), (7, 'Science fiction')]
    form.subcategory.choices = subcategories
...
def get_db():
    ...
    if db is None:
        ...
        print("DB is open now")
    return db


# decorator put the funciton at the end of the application (when request is over).
# requires exception parameter? why?
@app.teardown_appcontext
def close_connection(exception):
    db = getattr(g, "_database", None)
    if db is not None:
        db.close()
        print("db is close now")